...
if __name__ == "__main__":
    ...
    parser.add_argument("--agent1", type=str, default="simple", help='agent 1 name, only name, not path')
    ...
    # env = Environment(map_path, 'raw', 'raw', max_step=args.max_step, render=False,
    #                   random_pos=args.random_pos, log=log_flag, external_render=args.ext_render, side1_name=args.agent1,
    #                   side2_name=args.agent2)
    
    env = Environment(map_path, 'simple', 'simple', max_step=args.max_step, render=False,
                      random_pos=args.random_pos, log=log_flag, external_render=args.ext_render, side1_name=args.agent1,
                      side2_name=args.agent2)
    # get map info
    ...
    for x in range(args.round):
        ...
        while True:
            ...
            print('side1_obs_dict', side1_obs_dict)
            # get action
